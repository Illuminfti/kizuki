...
Typed receipt metadata is derived from the same one complete selected contribution per assertion as the image: weakest authority tier and rounded mean confidence across its assertions. For a deletion receipt, the recorded before-image supplies this operation metadata. The renderer is Core (`deterministic`, no model reference). Receipt provenance must equal the event union of both image bases; omitting an old image's events cannot hide it from purge. Shared image/basis verification enforces these bindings, including during export/restore and ordinary intent replay.

Typed receipt ownership is separate from image membership. An ordinary write may own only assertions in its after image which were absent from its before image, and must include every assertion whose recorded owner is this write (or is still unassigned at admission). Already receipted reconfirmations keep an empty ownership list. Later purge receipts may transfer surviving assertions without invalidating their earlier retained history. Reverts copy the exact recorded target's ownership and supersession list, invert its two bases and hashes, and require an undoable target on the same page; bounded ancestry validation rejects cycles. Restore/image validation and undo use the same checks, and undo validates before any publication or claim lifecycle mutation.

The retained typed redo of an absent page is also undoable without an archive: its exact shape is `kind: revert`, `page_action: create`, `before_hash: ABSENT_PAGE_HASH`, and `basis.before: null`. Undo deletes that new image, matching the same absent-before check used by the writer; other missing preimages still refuse. Repeated undo/redo therefore preserves absent-image semantics without inventing an archive.
