...
Ledger33 and canon5 add a discriminated representation in the existing receipt table and JSONL stream. Legacy v1 wire ordering, strict intent validation, row data, and hash bytes remain unchanged. Retained typed receipts have schema `kizuki.canon-receipt/v2`, state `retained`, own_id_origin `core`, `prior_receipt_id` (null or an opaque receipt ULID), the existing common receipt fields, and an exact basis:
...
Erased typed receipts use only `{schema,state:"erased",receipt_id,prior_receipt_id,purge_receipt_id,own_id_origin:"core",erased_at,sensitivity:"private",integrity}`. The integrity hash covers those fields in the `kizuki.canon-receipt/v2#erased` domain. Erasure preserves the exact original predecessor edge. They retain no path, claim/evidence identity, semantic handle, before/after content hash, archive, model reference, or basis. Undo returns an explicit erased refusal.

Typed receipt order comes from this single causal chain, never timestamps, ULID sorting, or equal content hashes. The bounded resolver admits at most 4,096 records per retained page lineage, follows erased bridges only with valid completed event-purge proofs, and rejects missing predecessors, forks, cycles, cross-page retained records, and disconnected retained roots. Adjacent retained records must agree on image hashes and exact before/after bases. A unique non-null predecessor index prevents forks. Undo, cascade, authority resolution, and index rebuild use the same chain; unavailable or erased predecessors cannot confer owner authority. New writes after an undone creation link the actual current absent-page receipt. Entirely erased histories without any retained page identity may detach.

Source/event erasure requires a separate version 3 arm in the existing canon intent mechanism: no captured preimage, exact prior hash during the unfinished operation, recorded purge proof, replacement bytes and independently revalidated survivor basis. Successful completion erases obsolete archives and retained receipt payload, leaving either a supported receipt without a preimage or an erased record and no page. The new record links the actual preceding receipt, including an erased predecessor, so earlier still-authorized history remains ordered. This arm, its recovery, backup/restore compatibility, and physical-erasure oracles must be complete before this implementation is integrated. This proposal does not adopt backup6 or unrelated proposed Purge6 contracts.
...
A typed materialization receipt acquires every newly rendered, previously unreceipted live assertion. Assertions already owned by earlier receipts remain only in the exact image bases; an edit does not reacquire their lifecycle. This makes single-claim invocation equivalent to the full generated page for undo, without reverting earlier assertions when a later correction is undone.

A revert records the restored image's actual sensitivity and taint and preserves its exact historical bytes. Current source sensitivity remains a separate serving and retrieval-publication floor. Retrieval upserts apply the current source floor even when a reverted historical page carries an earlier lower label. Typed ordinary admission and replay also reconstruct both recorded images from their exact admitted bases before native publication.
