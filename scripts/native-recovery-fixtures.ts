  ...
  for (const [index,id] of NATIVE_RECOVERY_PHASE_IDS.entries()) {
    ...
    try {
      if (id === "migrate-ledger15" || id === "migrate-ledger16") {
        ...
        const after = inspectRecoveryFixture(vault); evidence.snapshots.push({role:"restored",value:after.summary}); requireThat(after.summary.schema_version === 22 && after.summary.events === 1, "restored-shape");
