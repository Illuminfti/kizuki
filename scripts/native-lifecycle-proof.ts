...
function snapshot(value: unknown): Row {
  ...
  num(r.schema_version,1,LEDGER_SCHEMA_VERSION);hash(r.schema_sha256);hash(r.rows_sha256);hash(r.files_sha256);num(r.table_count,1,256);num(r.row_count,1,10000);num(r.events,0,10000);num(r.claims,0,10000);need(r.integrity==="ok"&&r.foreign_key_errors===0);return r;
}
function recovery(id: string,value: unknown,expected: NativeLifecycleIdentity): Row {
  ...
  if(id==="migration-failure-preserved") {
    ...
    need(last.schema_version===LEDGER_SCHEMA_VERSION&&last.events===preservation.events&&last.claims===preservation.claims);
...
function upgrade(value: unknown, expected: NativeLifecycleIdentity, prior: Row, platform:string): Row {
  ...
  need(e.baseline_schema===21&&e.candidate_schema===LEDGER_SCHEMA_VERSION);instance(e.baseline_instance_id);instance(e.candidate_instance_id);need(e.baseline_instance_id!==e.candidate_instance_id);
...
export function validateNativeLifecycle(value: unknown,expected:NativeLifecycleIdentity) {
  ...
  for(const s of services){instance(s.vault_id);instance(s.instance_id);num(s.pid,2,2**31-1);hash(s.event_text_sha256);need(s.ledger_schema===LEDGER_SCHEMA_VERSION&&s.active===true&&s.stopped===true);const u=vaultUnit(s.vault_id,s.unit,platform);need(s.event_text_sha256===(admitted.get(String(s.id))!.preservation as Row).event_text_sha256,"native-lifecycle-recovery-event-binding");need(!serviceIds.has(u));serviceIds.add(u);units.add(u);}
