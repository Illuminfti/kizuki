...
function hasMacNativeProof(document: Record<string, unknown>, job: Record<string, unknown>): boolean {
  ...
  return action(0, "actions/checkout", { "fetch-depth": 0, ref: "${{ github.event.pull_request.head.sha || github.sha }}" }) &&
    ...
    isConditionedCommand(steps[5], "bun run typecheck\nbun test scripts/release-targets.test.ts scripts/release-artifacts.test.ts scripts/stranger-proof.test.ts packages/core/test/serve/advisory-file-lock.test.ts packages/core/test/serve/flock.test.ts packages/core/test/serve/leases.test.ts packages/core/test/serve/units.test.ts packages/core/test/serve/service-arguments.test.ts packages/cli/test/config.test.ts packages/cli/test/terminal-prompt.test.ts packages/tui/test/terminal.test.ts packages/retrieval-pg/test/contention.test.ts scripts/native-platform.test.ts packages/core/test/vault-platform.test.ts packages/core/test/serve/durability.test.ts packages/cli/test/serve/install.test.ts\n" + NATIVE_CONSUMER_TESTS, FULL_QUALIFICATION) &&
