...
Policy date: 2026-09-01; amended 2026-09-02; amended 2026-09-04; evidence reconciled 2026-09-06
...
| --- | --- | --- | --- | --- | --- |
| [Beacon](https://github.com/Asymptote-Labs/agent-beacon) | Normalized local agent-run JSONL; source-attributed corrections and tool outcomes as evidence | MIT, Asymptote Labs 2026; preserve notices if code is later copied. No upstream code, fixtures or assets copied in this adapter. | Kizuki-owned local file adapter; no runtime dependency, hooks, install or hosted calls | Evaluated 2026-09-21 at `793524a9bae85bf5f7963da002e171c8bbebf136`, schema 1.0; synthetic Claude/Codex capture tests. See [scope and limits](beacon-import.md). | Gives the existing source-consented ledger attributable run evidence without creating a second learning store or promoting reported success into proof |
...
The owner requested deeper, coherent use of lessons from
[GBrain](https://github.com/garrytan/gbrain), beyond the already adopted ranking
recipe and graph. The [integration programme](retrieval-engine-integration.md)
and [acceptance charter](retrieval-engine-acceptance.md) attach those lessons to
#497 and existing implementation owners, with bounded follow-ons #527 and #528.
They specify work, not shipped functionality or live setup changes.

Source inspection is pinned to public `master` commit
`8c70f6255047a7647adb30b1d6333a48068d9fa5`, package `0.48.2.0`, rechecked on
6 September 2026. Kizuki inspection baseline is
`5e2958fff073bf8ac8ba446d09fc7b38faa3047f`. Paths in the table are relative to
the upstream repository at that immutable revision, not Kizuki files.

| Evidence | Inspected upstream source | Pattern to evaluate in Kizuki |
| --- | --- | --- |
| U1 | `docs/guides/push-context.md` | Bounded deterministic volunteered context, ambiguity gates, cross-turn deduplication and honest delivery/use diagnostics |
| U2 | `docs/protocol/MEMORY_VERBS_v1.md` | Small typed entity/context operations, scoped change delivery, budget packing, actionable errors and conformance distinct from quality |
| U3 | `src/core/search/rerank.ts`; `docs/ai-providers/llama-server-reranker.md` | Bounded reranking, preserved tail, explicit skipped/malformed/failure states and local endpoint integration |
| U4 | `src/core/think/index.ts` | Separate gathered evidence from model composition, sources and gaps; distinguish model failure from a real answer |
| U5 | `src/commands/dream.ts`; `docs/guides/minions-deployment.md` | Phase-selectable maintenance, bounded backlog work, supervision and explicit recovery limitations |
| U6 | `docs/schema-author-tutorial.md`; `skills/enrich/SKILL.md` | Useful typed entity views, declared schema vocabulary, scoped enrichment effort and evidence links |
| U7 | `docs/protocol/MEMORY_VERBS_v1.md`; `package.json` evaluation/test scripts | Executable conformance and separate quality evaluation; benchmark reports are hypotheses to reproduce, not inherited Kizuki scores |
| U8 | `docs/guides/bootstrap.md` | Client-specific capability reporting, verifiable setup, lifecycle integration and reversible configuration |

### Factual reconciliation without changing product law

The older D13 reconnaissance and this policy previously said that the upstream
had no reranker or local model path. That is not accurate for the inspected
public tip: U3 shows both a reranking implementation and a local HTTP-backed
reranker recipe. This appendix corrects the capability assessment, not the
historical decision rows. D17's narrower adoption remains unchanged: the vendored
code is still recipe-and-graph only, while reranking and local GGUF are
Kizuki-owned work. No in-process model runtime, model-weight compatibility,
performance advantage or production binding is established by source inspection.

### Integration and reuse boundaries

Borrow these behavioral patterns through existing Core contracts, retrieval
ports, source consent, agent grants, lifecycle adapters and the single receipted
writer. Preserve model-free recall and the distinction between expiry and
physical purge. No client receives owner authority merely by installing an
adapter, and a configured model endpoint does not authorize egress of local-only
source material. No raw-file fallback may bypass a denied Core read.

This documentation change adds no dependency, copied implementation, model
weights, upstream service, credential, scheduler, second canon writer or external
telemetry. It does not import upstream identity/bootstrap files into the owner's
workspace. Existing notices remain intact. Any further source reuse requires the
file-level revision/license/notices, dependency/egress, failure/removal and upgrade
record required above before it can be distributed. Favor narrow upstream fixes
when generally useful, and retain Kizuki-specific policy at the adapter boundary.

The seven tracks share a longitudinal acceptance scenario and 49 proposed cases.
They are not executed tests. Quality adoption requires matched baseline/ablation
receipts under #496; mock transports, real clients and human setup trials remain
separate evidence. A diagram, issue closure or upstream result never proves a
Kizuki runtime feature, benchmark win or release qualification.
