...
| [GBrain](https://github.com/garrytan/gbrain) | Hybrid retrieval recipe: reciprocal rank fusion at `k = 60`, layered near-duplicate post-filter, tier-weighted finalization, declared-degradation envelope | MIT, copyright 2026 Garry Tan; retain the copyright and permission notice for copied or substantial portions | Clean reimplementation of the retrieval recipe with prominent credit. A permitted fork remains open for the entity graph only. Not a Kizuki dependency today. The project is not published to a package registry Kizuki can depend on. | Evaluated 2026-09-01 at package 0.9.1, commit `aeb477d39b730ffc61c4435bc686c47618cd8e46`. That revision is a fork snapshot not reachable from any public upstream branch and is hundreds of commits behind the public tip. The public tip has a larger dependency surface. The engine has no reranker and no local GGUF path; those would be Kizuki work. | First non-trivial implementation target for `kizuki.retrieval/v1`. Credit lives only here and in the README. |
