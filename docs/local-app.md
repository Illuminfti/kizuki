...
The **Agents** view gives each assistant its own read-only access and explicit permissions. The initial sensitivity ceiling is public, so private imports require a deliberate higher ceiling. Review all eight grant fields, then copy the generated MCP configuration into that assistant and ask it to search an allowed source. It contains a private file reference, not a raw token or owner access. For two clients, create one access record for each. Revoke access ends that record's authority, including requests from an already-running MCP process.
