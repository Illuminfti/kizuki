...
The **Agents** view can enroll a read-only agent with its own name and explicit permissions. The initial sensitivity ceiling is public, so private imports require a deliberate higher ceiling. It shows stored authorization as active or revoked; that is not a claim that a client is connected or has successfully used it. Review all eight grant fields, then copy the generated MCP configuration into the assistant you use. It contains a private file reference, not a raw token or owner access. For two clients, create one access record for each and verify each with an allowed search; revoke access ends that record's authority, including requests from an already-running MCP process.
