...
Capture is bounded to twenty messages returned, twenty-five requests and forty-five seconds per method, with remaining-aware five-second request/state-persistence waits, bounded JSON, a 1000-message initial cap, an 8 KiB cursor and a 128 KiB pending plan. A bounded call captures one page and returns `has_more` when plan items remain or the next page token is non-null; the host continues remaining pages, including 404-only slices, in later calls. Initial caps, expired history and unavailable messages report partial coverage. Only provider-explicit deletion observations produce tombstones; provider deletion time is unknown, and absence does not infer deletion. Attachments are metadata only; body download, HTML interpretation and unsupported text encodings are not implemented. Minimal provider structure needed to parse a selected field is distinct from the fields persisted; see the connector README for projection details.
