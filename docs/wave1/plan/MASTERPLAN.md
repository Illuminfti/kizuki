...
deterministic with no LLM key. Superseded 2026-09-02, see
`docs/decision-log.md` D9, D10, D12: the loop writes canon autonomously with
a receipt per write, there is no staged digest to accept, and canon writing
requires a configured model while capture, ledger, search, timeline, context,
audit and undo stay model-free. `kizuki serve` turns on the always-on part:
...
Superseded 2026-09-02, see `docs/decision-log.md` C8 and D10: the owner
review gate is no longer a claim anywhere. The restated moat is autonomous,
provenance-total, reversible canon with conversational correction, zero
phone-home, any harness (RFC 0002 §1.3).
...
| 4   | Floors (Superseded in part 2026-09-02, see `docs/decision-log.md` D9, D10, D12: "owner-only promote" is dead, and the deterministic floor no longer covers canon writing) | All 9 standing calls + 19 evidence-settled positions carried (adapted to TS): no fake connectors, no public link pre-demo, owner-only promote, frozen thin ingress, subject ids day one, data preserved from declared floor, MCP fail-closed + capture = attacker-controlled, zero phone-home CI-enforced, upstream-first cutover; laundering gate, lessons-as-tests, purge receipts, secret_ref, deterministic floor, scripted releases, validated ontology + x- namespace, etc. |
| 5   | Agent layer (Superseded in part 2026-09-02, see `docs/decision-log.md` D14: writes are `propose` **and** `correct`, not propose-only into staging) | Agents are **first-class consumers**: per-agent identity, scoped read permissions with sensitivity ceilings, propose-only writes into staging, bounded context packets, full audit trail. No agent loop, no personas, no hosting — every harness brings its own                                                                                                                                                                                                                   |
| 6   | Run model (Superseded in part 2026-09-02, see `docs/decision-log.md` D15: `kizuki init` installs `kizuki serve` as an always-on user service; the CLI still runs when the daemon is down) | Program-first; `kizuki serve` starts the always-on part                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| 9   | Daily surface (Superseded 2026-09-02, see `docs/decision-log.md` D10, D14: the TUI is audit and undo only, and the daily surface is `kizuki tell` plus `kizuki audit`, not `kizuki review`) | CLI/TUI-first at 1.0; no web dashboard; notifier digests point at `kizuki review`                                                                                                                                                                                                                                                                                                                                                                                                 |
