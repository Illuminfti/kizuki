...
This is the first integration stage for issue 56. Export keeps its previous callback and SQL capture order. Cooperative file-writer exclusion does not establish a coherent snapshot against database-only writers or arbitrary progress callbacks. Callback-free capture and guarded publication remain separate work. Manual filesystem edits, full crash recovery, backup recovery authority, cross-platform native qualification, and release acceptance are also separate gates.
