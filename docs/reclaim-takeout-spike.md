...
projection. Runtime non-string input is refused without coercion, so a caller
type error cannot silently convert a value into text. It rejects input whose
UTF-8 encoding is lossy, such as lone
