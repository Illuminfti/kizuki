...
An unresolved acceptance case remains: Gmail live messages are mutable. An ID-only pending plan preserves deletion observations, but a live GET can change between a lost response and retry. A per-item version/content fingerprint fence is still needed to refuse changed live snapshots instead of claiming exact replay. Do not treat the passing unchanged-provider replay fixtures as proof of that case.
