...
OAuth refresh persistence is bound to the exact connected session generation and account. A timed-out refresh cannot write through a later connection, including a newer token generation of the same account. State-persistence waits are capped at five seconds and the remaining capture budget; timeout returns a fixed `timeout` diagnostic. An already-started host write cannot be canceled: reconnect and sign-in refuse until that promise settles, then reconnect must reload durable state. A never-settling host write therefore leaves this instance unavailable, rather than allowing unsafe replacement. This uses the existing persister and introduces no token store.
