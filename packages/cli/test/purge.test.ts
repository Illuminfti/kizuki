...
describe("RFC 0002 §16.4 purge and undo", () => {
  test("purge --record holds the page and --verify prints absence proofs", async () => {
    ...
    // Import files a live claim for acme.md; insertClaim adds a second.
    // Those two plus the event and page are the four absence proofs.
    expect(verified.stdout).toMatch(
      /kizuki\.retrieval\.fts5\s+checked 4\s+found 0\s+ok/,
    );
