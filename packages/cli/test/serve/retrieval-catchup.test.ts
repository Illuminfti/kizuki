...
test("retrieval-sweep catch-up lets query succeed without --degraded", async () => {
  ...
  const receipt = JSON.parse(sweep.stdout) as { data: { status: string; retrieval: { upserts: number } } };
