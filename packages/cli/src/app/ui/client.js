...
const state = { view: 'memory', status: null, service: null, model: null, modelError: false, agents: null, agentsError: false, sources: [], catalog: [], receipts: [], activityStatus: 'idle', hits: null, query: '', degraded: [], busy: false, operation: null, dismissedCaptureId: null, setupError: null, world: null, worldError: false, worldKind: 'concepts', worldQuery: '' };
let bearer = null;
let noticeTimer;
...
function humanError(code) {
  if (code === 'recovery_pending') return 'Recovery remains pending. Run kizuki recover --json in a terminal. Inspect any unknown external operation before repeating this change.';
...
function renderMemory() {
  ...
  if (state.busy) section.append(el('div', { class: 'opening', 'aria-busy': 'true', 'aria-label': 'Searching your memory' }, el('div', { class: 'skeleton skeleton-line' }), el('div', { class: 'skeleton skeleton-panel' })));
    ...
    for (const hit of state.hits) {
      ...
      list.append(el('article', { class: 'result-item' }, el('div', { class: 'result-meta' }, el('span', { class: 'badge' }, hit.scope === 'canon' ? 'Memory page' : 'Source evidence'), hit.scope === 'canon' && hit.taint === 'quoted' && el('span', { class: 'badge' }, 'Includes quoted evidence'), el('span', {}, hit.sensitivity === 'public' ? 'Public' : hit.sensitivity === 'internal' ? 'Internal' : 'Private')), el('h3', {}, resultTitle(hit)), subjects, el('p', { class: 'result-text' }, hit.text), el('details', { class: 'result-details' }, el('summary', {}, 'View evidence references'), el('p', {}, 'Use these references to check the original information.'), ...citations.map(id => el('p', {}, el('code', {}, id)))), hit.scope === 'canon' && button('Correct memory', () => correction(hit))));
    }
    section.append(list);
  }
