    ...
    async function execute(route: AppRoute, input: Record<string, unknown>): Promise<unknown> {
        ...
        if (route === 'query') {
            ...
            return readContext(async (ctx) => {
                ...
                return { hits: [...result.canon.map(hit => ({ id: hit.page_id, scope: 'canon', title: hit.title, text: hit.excerpt, citations: hit.sources, sensitivity: hit.sensitivity, ...(hit.subject_labels === undefined ? {} : { subject_labels: hit.subject_labels }) })), ...result.quoted.map(hit => ({ id: hit.event_id, scope: 'ledger', title: hit.connector_id, text: hit.text, citations: [...new Set([hit.event_id, ...(hit.subject_labels ?? []).flatMap(label => label.evidence.flatMap(item => item.sources))])], sensitivity: hit.sensitivity, ...(hit.subject_labels === undefined ? {} : { subject_labels: hit.subject_labels }) }))], withheld: result.denied.reduce((n, item) => n + item.count, 0), degraded: result.data?.degraded ?? [] };
            }, true);
        }
        if (route === 'activity') {
