...
export async function runGmailConnect(io: CliIo, options: GmailEnrollmentOptions, checkSensitivity: (db: Database, manifest: Manifest, requested: Sensitivity | undefined, connection?: Connection) => void, create: GmailFactory = createGmailConnector, openUrl: (url: string) => Promise<void> = openGmailBrowser): Promise<number> {
    ...
    if (!io.stdinIsTTY || !io.stderrIsTTY)
        throw new UsageError('connect gmail --fields FIELDS [--source KEY] [--json] (interactive desktop terminal required)');
    return withVault(io, async (ctx) => {
        ...
        try {
            connection = await enrollSignedInConnection(ctx.db, ctx.store, connector, { prompt: async () => { throw new ConnectionError('Gmail does not request pasted keys or authorization codes.'); }, notify: () => { }, openUrl }, options.source, assertSameGmailIdentity);
        }
        catch {
