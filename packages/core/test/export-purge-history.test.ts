...
describe("completed purge history backup", () => {
  ...
  test("event-only selector provenance survives backup restore", async () => {
    ...
    const leftover = f.event("atlas-three");
    expect(leftover.connector_id).toBe("fixture");
    ...
    await runPurge(f.db, f.vault, { connector_id: "fixture" }, "retire fixture");
