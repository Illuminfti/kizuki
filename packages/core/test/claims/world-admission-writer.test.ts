...
function fixture(withWorldTables: boolean) {
  ...
  return { db, eventId: accepted.event.event_id };
}

function semantic(eventId: string): ClaimV2Assertion {
  return { schema: CLAIM_V2_SCHEMA, discriminator: "assertion", subject: { kind: "occurrence", id: "grace" }, predicate: "role.holds",
...
test("production insertion writes typed world meaning, derived admission, and endpoint receipts atomically", async () => {
  const { db, eventId } = fixture(true);
  try {
    const first = await insertClaim({ db, now: () => "2026-01-02T00:00:00.000Z" }, input(eventId));
    ...
    const repeated = await insertClaim({ db, now: () => "2026-01-02T00:00:00.000Z" }, input(eventId));
...
test("qualified world admission rolls back when migration 32 is absent", async () => {
  const { db, eventId } = fixture(false);
  try {
    await expect(insertClaim({ db }, input(eventId))).rejects.toThrow("migration_required");
...
test("a native correction is a separate support contribution and forged native support fails", async () => {
  const { db, eventId } = fixture(true);
  try {
    const initial = await insertClaim({ db }, input(eventId));
    ...
    const next = semantic(correction.event_id);
    ...
    const result = await insertClaim({ db }, {
      ...input(correction.event_id), body: "Grace now leads strategy.", semantic: corrected,
      ...
      world_admission: { ...input(correction.event_id).world_admission, semantic: corrected, rendering: { body: "Grace now leads strategy.", frontmatter: {} } },
    });
    ...
    await expect(insertClaim({ db }, {
      ...input(forged.event.event_id), provenance: [forged.event.event_id], producer: "owner", intent: "correct",
