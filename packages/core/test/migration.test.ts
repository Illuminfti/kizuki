...
import { applyEventPurgeIntegrityV22, applyEventPurgeSelectorKindV24, applyEventPurgeSelectorKindV26, applyEventPurgeSelectorKindV27, applyEventPurgeSelectorKindV28, eventPurgeProofDigest } from "../src/ledger/purge-schema";
import {
...
describe("openLedger migrations", () => {
  ...
  test("v28 widens selector_kind to source and binds stored proof bytes without rewriting proofs", () => {
    ...
    const fresh = openLedger(":memory:");
    expect(schemaVersion(fresh)).toBe(LEDGER_SCHEMA_VERSION);
    expect(schemaVersion(fresh)).toBe(28);
