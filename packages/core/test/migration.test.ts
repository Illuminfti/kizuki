...
import { applyEventPurgeIntegrityV22, applyEventPurgeSelectorKindV24, applyEventPurgeSelectorKindV26, applyEventPurgeSelectorKindV27, applyEventPurgeSelectorKindV28, applyEventPurgeSelectorKindV29, applyEventPurgeSelectorKindV30, eventPurgeProofDigest } from "../src/ledger/purge-schema";
import {
...
describe("openLedger migrations", () => {
  ...
  test("v26 widens selector_kind to connector without rewriting proofs", () => {
    const db = new Database(":memory:");
    db.exec(`
      CREATE TABLE event_purges (
        receipt_id TEXT PRIMARY KEY,
    ...
    expect(() =>
      db.query(
        `INSERT INTO event_purge_proofs (receipt_id, content_hash, source_record_id, selector_kind)
         VALUES ('01JCPURGEPROOF0000000000001', ?, 'mail-record', 'connector')`,
      ).run("b".repeat(64)),
    ).not.toThrow();
    db.query(
  ...
  test("v28 widens selector_kind to source and binds stored proof bytes without rewriting proofs", () => {
    const db = new Database(":memory:");
    db.exec(`
      CREATE TABLE event_purges (
        receipt_id TEXT PRIMARY KEY,
    ...
    applyEventPurgeSelectorKindV26(db);
    applyEventPurgeSelectorKindV27(db);
    applyEventPurgeSelectorKindV28(db);
    applyEventPurgeSelectorKindV28(db);
    expect(tableColumns(db, "event_purges")).toContain("proof_digest");
    expect(
      db.query<{ selector_kind: string | null; content_hash: string; source_record_id: string }, []>(
        "SELECT selector_kind, content_hash, source_record_id FROM event_purge_proofs",
      ).get(),
    ).toEqual({
      selector_kind: null,
    ...
    expect(
      db.query<{ proof_digest: string | null }, []>("SELECT proof_digest FROM event_purges").get(),
    ).toEqual({
      proof_digest: eventPurgeProofDigest("a".repeat(64), "legacy-record", null),
    });
    db.query(
      `INSERT INTO event_purges (receipt_id, event_id, connector_id, reason, purged_at)
       VALUES ('01JCPURGEPROOF0000000000001','01JCPURGEEVENT0000000000001','mail','legacy','2026-09-06T12:00:00.000Z')`,
    ).run();
    expect(() =>
      db.query(
        `INSERT INTO event_purge_proofs (receipt_id, content_hash, source_record_id, selector_kind)
         VALUES ('01JCPURGEPROOF0000000000001', ?, 'mail-record', 'source')`,
      ).run("b".repeat(64)),
    ).not.toThrow();
    db.query(
      `INSERT INTO event_purges (receipt_id, event_id, connector_id, reason, purged_at)
       VALUES ('01JCPURGEPROOF0000000000002','01JCPURGEEVENT0000000000002','mail','legacy','2026-09-06T12:00:00.000Z')`,
    ).run();
    expect(() =>
      db.query(
        `INSERT INTO event_purge_proofs (receipt_id, content_hash, source_record_id, selector_kind)
    ...
    const fresh = openLedger(":memory:");
    expect(schemaVersion(fresh)).toBe(LEDGER_SCHEMA_VERSION);
    expect(schemaVersion(fresh)).toBe(30);
    expect(tableColumns(fresh, "event_purges")).toContain("proof_digest");
    fresh.close();
    db.close();
  });

  test("v29 widens selector_kind to subject without rewriting proofs", () => {
    const db = new Database(":memory:");
    db.exec(`
      CREATE TABLE event_purges (
        receipt_id TEXT PRIMARY KEY,
    ...
    applyEventPurgeSelectorKindV27(db);
    applyEventPurgeSelectorKindV28(db);
    applyEventPurgeSelectorKindV29(db);
    applyEventPurgeSelectorKindV29(db);
    expect(
      db.query<{ selector_kind: string | null; content_hash: string; source_record_id: string }, []>(
        "SELECT selector_kind, content_hash, source_record_id FROM event_purge_proofs",
    ...
    expect(() =>
      db.query(
        `INSERT INTO event_purge_proofs (receipt_id, content_hash, source_record_id, selector_kind)
         VALUES ('01JCPURGEPROOF0000000000001', ?, 'mail-record', 'subject')`,
      ).run("b".repeat(64)),
    ).not.toThrow();
    db.query(
      `INSERT INTO event_purges (receipt_id, event_id, connector_id, reason, purged_at)
    ...
    expect(() =>
      db.query(
        `INSERT INTO event_purge_proofs (receipt_id, content_hash, source_record_id, selector_kind)
         VALUES ('01JCPURGEPROOF0000000000002', ?, 'mail-record', 'alias')`,
      ).run("c".repeat(64)),
    ).toThrow();
    const fresh = openLedger(":memory:");
    expect(schemaVersion(fresh)).toBe(LEDGER_SCHEMA_VERSION);
  ...
  test("v30 widens selector_kind to event+connector without rewriting proofs", () => {
    const db = new Database(":memory:");
    db.exec(`
      CREATE TABLE event_purges (
        receipt_id TEXT PRIMARY KEY,
    ...
    applyEventPurgeSelectorKindV28(db);
    applyEventPurgeSelectorKindV29(db);
    applyEventPurgeSelectorKindV30(db);
    applyEventPurgeSelectorKindV30(db);
    expect(
      db.query<{ selector_kind: string | null; content_hash: string; source_record_id: string }, []>(
        "SELECT selector_kind, content_hash, source_record_id FROM event_purge_proofs",
    ...
    expect(() =>
      db.query(
        `INSERT INTO event_purge_proofs (receipt_id, content_hash, source_record_id, selector_kind)
         VALUES ('01JCPURGEPROOF0000000000001', ?, 'mail-record', 'event+connector')`,
      ).run("b".repeat(64)),
    ).not.toThrow();
    db.query(
      `INSERT INTO event_purges (receipt_id, event_id, connector_id, reason, purged_at)
  ...
  test("v28 binds historical proof bytes when extending an unreleased selector-only schema", () => {
    const directory = mkdtempSync(join(tmpdir(), "kizuki-ledger-v28-digest-"));
    const path = join(directory, "ledger.sqlite");
    try {
      ...
      db.query(
        `INSERT INTO event_purge_proofs (receipt_id, content_hash, source_record_id, selector_kind)
         VALUES ('01JCPURGEPROOF0000000000000', ?, 'legacy-record', NULL)`,
      ).run("a".repeat(64));
      db.exec("ALTER TABLE event_purges DROP COLUMN proof_digest");
      db.query("UPDATE schema_version SET version = 27").run();
      expect(tableColumns(db, "event_purges")).not.toContain("proof_digest");
      db.close();
      const upgraded = openLedger(path);
      expect(schemaVersion(upgraded)).toBe(LEDGER_SCHEMA_VERSION);
      expect(tableColumns(upgraded, "event_purges")).toContain("proof_digest");
      expect(
        upgraded.query<{ selector_kind: string | null }, []>(
          "SELECT selector_kind FROM event_purge_proofs",
        ).get(),
      ).toEqual({ selector_kind: null });
      expect(
        upgraded.query<{ proof_digest: string | null }, []>("SELECT proof_digest FROM event_purges WHERE receipt_id = '01JCPURGEPROOF0000000000000'").get(),
      ).toEqual({
        proof_digest: eventPurgeProofDigest("a".repeat(64), "legacy-record", null),
      });
      upgraded.close();
