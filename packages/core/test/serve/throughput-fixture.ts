...
export function segmentModelProducer(
  vault: string,
  /** Runs while each request is in flight; "malformed" answers it with a completion that is not a typed response. */
  onRequest: (request: number) => void | "malformed" = () => undefined,
  ...
  const llm: LlmPort = {
    ...
    async complete(request) {
      ...
      if (onRequest(requests.length) === "malformed") {
        return { text: "{\"schema\":", model: MODEL, usage: { input_tokens: 10, output_tokens: 3 } };
      }
