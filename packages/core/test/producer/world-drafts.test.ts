...
const supplied: QualifiedSuppliedRef = { kind: "supplied", id: "subject-1", namespace: { connector_id: "fixture", source_key: sourceKey } };
...
const context: WorldDraftContext = { events: [{ ...event, source_key: sourceKey, text: input.events[0]!.text, subjects: [{ kind: "supplied", id: "subject-1" }] }], supplied_refs: new Map([["s0", supplied]]), model_ref: "fixture-model" };
...
test("rejects forged supplied subjects, unsupported mentions, and anchors outside the selected event", () => {
  expect(() => prepareWorldDrafts({ ...response, claims: [{ ...response.claims[0]!, subject: { kind: "supplied", id: "s0" } }] }, input, { ...context, supplied_refs: new Map([["s0", { ...supplied, id: "forged" }]]) })).toThrow("absent from its cited event");
...
test("supplied subject witnesses cannot cross connector or source namespaces", () => {
  ...
  for (const namespace of [
    ...
    expect(() => prepareWorldDrafts(direct, input, { ...context, supplied_refs: new Map([["s0", { ...supplied, namespace }]]) })).toThrow("absent from its cited event");
  }
});

test("resolved context uses canonical reference order rather than locale collation", () => {
  const upper = { ...supplied, id: "Z" }, lower = { ...supplied, id: "a" };
  const extendedInput = { ...input, supplied_refs: [...input.supplied_refs, { id: "s1", anchors: [anchor] }, { id: "s2", anchors: [anchor] }] };
  const extendedContext: WorldDraftContext = { ...context,
    events: [{ ...context.events[0]!, subjects: [...context.events[0]!.subjects, upper, lower] }],
