...
describe("strict extraction schema", () => {
  ...
  test("Markdown code fences are rejected instead of relaxed into JSON", () => {
    const text = "```json\n" + responseText([draft()]) + "\n```";
    expectInvalid(text, "not JSON");
  });
  ...
  test("control, line-separator, and bidi characters never survive into drafts", () => {
    expectInvalid(
      responseText([draft({ object: "acme\u0000corp" })]),
      "object",
    );
    expectInvalid(
      responseText([draft({ subject: "acme-mail:grace\u202e" })]),
      "subject",
    );
    expectInvalid(
      responseText([draft({ body: "one\u2066two\u2069" })]),
      "body",
    );
    expectInvalid(
      responseText([draft({ object: "one\u2028two" })]),
      "object",
    );
  });
