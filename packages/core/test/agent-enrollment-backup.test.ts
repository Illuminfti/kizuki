...
for (const schema of ["kizuki.backup/v2", "kizuki.backup/v3"] as const) {
  for (const ledger of schema === "kizuki.backup/v3" ? [16, 17, 18, 19, 20, 21, 22, 23] : [16, 17, 18, 19, 20]) {
  ...
  for (const ledger of [0, 15, ...(schema === "kizuki.backup/v2" ? [21, 22, 23] : [24]), 99, 16.5, "16"]) {
