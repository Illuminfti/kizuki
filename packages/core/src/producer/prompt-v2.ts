...
export const EXTRACTION_V2_SYSTEM_PROMPT = [
  ...
  "Temporal basis is explicit, observed, or unknown. Known valid_from and valid_to are RFC3339 timestamps with an exclusive end; unknown time uses both null. Explicit basis requires valid_from; a deadline or due date alone is not a validity interval, so keep it in the literal and use null/null/unknown. Emit at most 64 mentions and 128 claims, 8 anchors per item, 400 characters per literal and 1200 per body. Return empty arrays when no grounded claim exists.",
