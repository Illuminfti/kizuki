...
export type ExtractMine =
  ...
  /**
   * One record passed over without claims. A record too large for one request
   * whose text has no safe split also carries `MineResult.skipped`, its retry receipt.
   */
 ...
 * every input still in the ledger, deferred input still queued, and a segment
 * still starting where its record's filed text ends. An answer that arrives
 * after a purge or another pass changed that state is discarded.
 */
export function extractDecisionCurrent(db: Database, mined: MineResult): boolean {
  ...
  if (mined.segment !== undefined && segmentStart(db, mined.segment.event_id) !== mined.segment.start) return false;
