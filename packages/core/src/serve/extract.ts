...
export async function mineLiveDrafts(
  ...
  if (batch.events.length === 0 || batch.cursor === null) {
    return { mined: { status: "empty" }, drafts: [], usage: NO_USAGE, previous_cursor, cursor: null };
  }
  ...
  if (usable.length === 0) {
    return { mined: { status: "empty" }, drafts: [], usage: NO_USAGE, previous_cursor, cursor: batch.cursor, input_ids: batch.events.map(event => event.event_id) };
  }
  ...
  return { mined, drafts, usage, previous_cursor, cursor: batch.cursor, input_ids: batch.events.map(event => event.event_id) };
}
