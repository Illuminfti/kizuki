...
export function readDerivedHolds(db: Database, pages: readonly CanonPage[] = []): {
  ...
  const paths = new Set(tableExists(db, "canon_holds")
    ...
    : []);
  for (const row of db.query<{ page_path: string }, []>(
    "SELECT page_path FROM canon_write_intents UNION SELECT page_path FROM canon_projection_obligations",
  ).all()) paths.add(row.page_path);
  ...
  if (tableExists(db, "page_index")) {
    for (const row of db.query<{ page_id: string }, []>(
      "SELECT page_id FROM page_index WHERE rel_path IN (SELECT page_path FROM canon_holds UNION SELECT page_path FROM canon_write_intents UNION SELECT page_path FROM canon_projection_obligations)",
  ...
  if (tableExists(db, "search_documents")) {
    for (const row of db.query<{ doc_id: string }, []>(
      "SELECT doc_id FROM search_documents WHERE scope='canon' AND path IN (SELECT page_path FROM canon_holds UNION SELECT page_path FROM canon_write_intents UNION SELECT page_path FROM canon_projection_obligations)",
    ).all()) {
