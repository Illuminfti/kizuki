...
export function validityOverlaps(
  ...
  // RFC 0002 §5.2: overlap is on instants; an omitted end is +∞.
  const leftStartsBeforeRightEnds =
    right.valid_to === null ||
    compareRfc3339(left.valid_from, "valid_from", right.valid_to, "valid_to") < 0;
  const rightStartsBeforeLeftEnds =
    left.valid_to === null ||
    compareRfc3339(right.valid_from, "valid_from", left.valid_to, "valid_to") < 0;
