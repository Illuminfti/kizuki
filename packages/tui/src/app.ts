...
function absentHash(): string {
  return new Bun.CryptoHasher("sha256").update(new Uint8Array()).digest("hex");
}
...
function readBody(
  ...
  if (!existsSync(path)) return { body: null, error: null };
  try {
    const raw = readFileSync(path, "utf8");
    try {
      return { body: parseFrontmatter(raw).body, error: null };
    } catch (error) {
      const message = error instanceof Error ? error.message : "unreadable page";
      return { body: raw, error: `unreadable page: ${message}` };
    }
  } catch (error) {
    const message = error instanceof Error ? error.message : "unreadable page";
    return { body: null, error: `unreadable page: ${message}` };
  }
}
...
function pageHash(vaultPath: string, relPath: string): string {
  const path = join(vaultPath, relPath);
  return fileHash(path) ?? absentHash();
}
...
export async function runAudit(opts: AuditOptions): Promise<AuditSummary> {
  ...
  const runEffect = async (effect: Effect): Promise<boolean> => {
    switch (effect.type) {
      ...
      case "undo": {
        const disk = pageHash(opts.vaultPath, effect.pagePath);
