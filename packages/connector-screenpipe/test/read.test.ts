...
describe("screenpipe row readers", () => {
  test("an oversized row is bounded before it reaches memory", () => {
    ...
    try {
      ...
      expect(frame?.window_name).toHaveLength(MAX_TEXT_CHARS + 2);
      expect(frame?.app_name).toHaveLength(MAX_TEXT_CHARS + 2);
      expect(spoken?.transcription).toHaveLength(MAX_TEXT_CHARS + 2);
      expect(spoken?.device).toHaveLength(MAX_TEXT_CHARS + 2);
    } finally {
      db.close();
    }
  });
  ...
  test("astral text is bounded in code units, not code points", () => {
    ...
    try {
      ...
      for (const value of [
        ...
        expect(value?.length).toBeLessThanOrEqual(MAX_TEXT_CHARS + 2);
        expect(value?.length).toBeGreaterThan(MAX_TEXT_CHARS);
