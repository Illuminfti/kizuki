...
`agent`, `audit`, `tell`, `undo`, `query`, `doctor`, `serve`, `purge`,
`export`, `version`. `review`, `promote`, and `reject` are retired: they
exit 2 and point at `audit`, `undo`, and `tell`.
...
| Manage scoped agent identities | `agent add\|list\|revoke\|rotate` |
