...
from .adapters import defaults, statuses
...
def main(argv=None):
 ...
 try:
  ...
  if a.cmd=="init":
   ...
   st.claim_controller(); st.verify_integrity(); Guard(limits).check(st); port=a.port or cfg["observer"]["port"]; srv=serve(st,adapters,host=cfg["observer"]["host"],port=port,guard=Guard(limits)); print(json.dumps({"host":"127.0.0.1","port":port,"observer_only":True,"controller_epoch":st.claimed_epoch}));
